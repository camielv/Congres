...
if(strlen($get) > 0 && preg_match("/^[a-z0-9' ]+$/", $get) != 0) {
	...
	if($ticket) {
		...
		if( $_POST['submit'] ) {
			...
			$message = "<p>Status: Your preferences are succesfully saved!</p>";
			for($i = 1; $i < 5; $i++) {
				if( $_POST['it' . $i] == "on" && $_POST['human' . $i] != "on" ) {
					...
					$ticket['slot' . $i] = "";
					$message = "<p>Status: You can only select one select one lecture</p>";
				}
			}
...
<form method="post">
<label for="name">Full Name:</label><input class="pref" name="name" type="text" <?php if($ticket['name']) echo "value=\"" . $ticket['name'] . "\"";?>/>
<table class="boxed">
	<tr>
		<th></th><th></th>
		<th>
